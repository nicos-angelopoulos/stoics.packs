...
% * 
% * cname(Cnm)         returns the column name of Cid
% * column(Clm)        returns the values of Cid in Mtx (does not include Cnm)
% * cposition(Cps)     returns the column position of Cid
% * mtx(Mtx)           returns the cannonical representation of Mtx
% * names(Names=[])    defines heatmap's columns names, either a list or a column identifier in Mtx
% * outputs(Outs=x11)  a single or list of [x11,pdf], see r_call/2
% * privates(Prv=false) if true, include private Ropts
% * rvar(Rvar=mcp)     R variable to use for the data
% * rvar_rmv(Rdel=true) delete the Rvariable at end of call   fixme: not implemented yet...
% * stem(Stem)         stem for output files. Default: atomic_list_concat([Cnm,phmap],'_',Stem).
%
% Ropt
% * breaks=                allows specific colour index assignments
% * cellheight=8
% * cellwidth=8
% * cluster_rows='FALSE'   as this is for single row
% * cluster_cols='FALSE'   ditto
% * fontsize_col=4
% * fontsize_row=4
% * fontsize=6
% * legend=Legend          'TRUE' if Prv=true and 'FALSE' otherwise
% * legend_horiz='TRUE'    private (only include if privates(true)
% * legend_hf=0.4          private
% * legend_wf=0.7          private
% * scale="none" 
% 
%==
