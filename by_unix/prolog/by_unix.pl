...
The library uses three local flags if they are defined.

==
    current_prolog_flag( by_unix_shell, Shell )
    current_prolog_flag( by_unix_shell_com_arg, CmA )
    current_prolog_flag( by_unix_surpress_fail_message, FailMess )
==
...
FailMess is false by default. Turn to true, to suppress failure messages
reporting when the shell fails.
...
Surpressing the failure report: 
==
?- @'R'('--slave','--no-echo','-e', library(set), stderr(null) ).
% Failed shell call: R with args: [--slave,--no-echo,-e,library(set)].
false.

?- set_prolog_flag( by_unix_surpress_fail_message, true ).
true.
...
unix_process( Name, _Goal, Args, _Opts ) :-
    ( current_prolog_flag(by_unix_surpress_fail_message, true) ->
          true
          ;
